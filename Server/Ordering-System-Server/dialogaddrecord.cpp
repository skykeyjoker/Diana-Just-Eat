...
DialogAddRecord::DialogAddRecord(QWidget *parent) : QDialog(parent) {
	...
	//选择图片按钮
	connect(btn_Photo, &QPushButton::clicked, [&, this]() {
...
void DialogAddRecord::slotBtnSubmitClicked() {
	...
	//拷贝图片到程序运行目录
	QString newPath = QDir::currentPath() + "/Pic/" + le_Photo->text().mid(le_Photo->text().lastIndexOf("/") + 1, -1);
	QFileInfo info(newPath);
	QString fileName = info.fileName();

	qDebug() << picPath << newPath << fileName;
